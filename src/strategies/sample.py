...
def buyStock(order):
	...
	if  (
		...
		int(data.get_avg_volume(order.ticker)) < volumeCap
		):
...
def sellStock(order):
	...
	if  (
		...
		float(data.get_price(order.ticker)) > data.get_50day_moving_avg(order.ticker) or
		float(data.get_price(order.ticker)) < float(order.buyPrice) * float(1 - lossMargin)
		):
